...
# This script conduct Harmonisation for subprocess, produce IV Output in two-sample MR format
...
Harmonisation = function(exposure_list, outcome_trait){ 
	...
	for (exposure_trait in exposure_list){
		...
		if (outcome_trait=='NAFL_UKBB'){
			proxy_snp = proxy_snp_nafl
		} else if (outcome_trait=='NASH_FinnGen'){
			proxy_snp = proxy_snp_nash
		} else if (outcome_trait=='LF_FinnGen'){
			proxy_snp = proxy_snp_lf
		} else if (outcome_trait=='Cirrhosis_FinnGen'){
			proxy_snp = proxy_snp_cirrhosis
		} else if (outcome_trait=='HCC_FinnGen'){
			proxy_snp = proxy_snp_hcc
		}
		...
		write.table(dat, paste0('./Project/',Project, '/subprocess/', exposure_trait, '_to_', outcome_trait, '_instruments.csv'),
...
proxy_snp_1 = read_excel('/home/dongzechen/GWAS/04-twosampleMR/Manual_IVs/Biomarkers_NAFLD_Project/proxy_snp_nafl.xlsx')
proxy_snp_nafl = proxy_snp_1$need_proxy; names(proxy_snp_nafl) = proxy_snp_1$proxy_snp

proxy_snp_2 = read_excel('/home/dongzechen/GWAS/04-twosampleMR/Manual_IVs/Biomarkers_NAFLD_Project/proxy_snp_nash.xlsx')
proxy_snp_nash = proxy_snp_2$need_proxy; names(proxy_snp_nash) = proxy_snp_2$proxy_snp

proxy_snp_3 = read_excel('/home/dongzechen/GWAS/04-twosampleMR/Manual_IVs/Biomarkers_NAFLD_Project/proxy_snp_lf.xlsx')
proxy_snp_lf = proxy_snp_3$need_proxy; names(proxy_snp_lf) = proxy_snp_3$proxy_snp

proxy_snp_4 = read_excel('/home/dongzechen/GWAS/04-twosampleMR/Manual_IVs/Biomarkers_NAFLD_Project/proxy_snp_cirrhosis.xlsx')
proxy_snp_cirrhosis = proxy_snp_4$need_proxy; names(proxy_snp_cirrhosis) = proxy_snp_4$proxy_snp

proxy_snp_5 = read_excel('/home/dongzechen/GWAS/04-twosampleMR/Manual_IVs/Biomarkers_NAFLD_Project/proxy_snp_hcc.xlsx')
proxy_snp_hcc = proxy_snp_5$need_proxy; names(proxy_snp_hcc) = proxy_snp_5$proxy_snp


outcome_list = c('NAFL_UKBB', "NASH_FinnGen", "LF_FinnGen", "Cirrhosis_FinnGen", "HCC_FinnGen")
