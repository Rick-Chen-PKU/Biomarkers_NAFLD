...
# This script conduct main two-sample MR for clinical spectra
...
TSMR <- function(exposure_list, outcome_trait){ 
	for (exposure_trait in exposure_list){
		dat = data.frame(fread(paste0('./Project/', Project, '/subprocess/', exposure_trait, '_to_', outcome_trait, '_instruments.csv')))  
		if (sum(dat$mr_keep) >= 2) {
			...
			if (nrow(dat) >= 4){
				res3 = mr_presso(BetaOutcome = "beta.outcome", BetaExposure = "beta.exposure", SdOutcome = "se.outcome", SdExposure = "se.exposure", 
					OUTLIERtest = TRUE, DISTORTIONtest = TRUE, data = dat, NbDistribution = 1000,  SignifThreshold = 0.05, seed=142857)
			...
			if (file.exists(paste0('./', Project, '_MR_Result_subprocess.xlsx'))){
				old_dat = read_excel(paste0('./', Project, '_MR_Result_subprocess.xlsx'))
				new_dat = rbind(old_dat,"",out_df)
				write_xlsx(new_dat, paste0('./', Project, '_MR_Result_subprocess.xlsx')) 
			} else {
				write_xlsx(out_df, paste0('./', Project, '_MR_Result_subprocess.xlsx')) 
			} 
